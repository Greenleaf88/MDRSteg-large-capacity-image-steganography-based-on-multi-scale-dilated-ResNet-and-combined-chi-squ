  ...
  try:
    ...
    if all_tensors or all_tensor_names:
      ...
      for key in sorted(var_to_shape_map):
        ...
        if all_tensors:
          # if key.split('/')[-1].startswith('moving'):
          if 'beta1' in key:
            #pass
          ...
          # mean = data.mean()
          # std = data.std()
          # fig = plt.hist(data.reshape([-1]), bins=8)
          # fig[2][0].figure.savefig(os.path.dirname(file_name)+'/'+'-'.join(key.split('/'))+'.png')
          # plt.clf()
          # print(data.shape, mean, std, data.max(), data.min())
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "--file_name",
      type=str,
      default="logs/00-gray/0417-1820/-55392",
  ...
  # print_tensors_in_checkpoint_file('logs/0407-2119/-50', tensor_name='', all_tensors=True)
